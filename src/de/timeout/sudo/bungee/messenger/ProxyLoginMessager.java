...
public class ProxyLoginMessager extends AbstractPluginMessager {
	...
	public ProxyLoginMessager() {
		super("login");
		System.out.println("ProxyLoginMessager load");
		// get uuid of proxy
		try {
			uuid = UUID.fromString(
					ConfigurationProvider.getProvider(YamlConfiguration.class)
						.load(new File(main.getDataFolder().getParentFile().getParentFile(), "config.yml")).getString("stats"));
		} catch (IOException e) {
...
//		System.out.println("Message empfangen von Tag: " + event.getTag());
//		// get Sender
...
//						
//						sendPluginMessage("login", server, stream.toByteArray());
//					}
//				} catch (IOException e) {
...
//	}
	...
	public void onPluginMessageReceived(PluginMessageEvent event) {
		Sudo.log().log(Level.INFO, "Test");
	}
	
	@EventHandler
	public void test(PluginMessageEvent event) {
		Sudo.log().log(Level.INFO, "Test inside subclass");
	}
}
